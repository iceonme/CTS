...
| 参赛者 | 优先级 | 说明 |
|--------|--------|------|
| **DCA 定投选手** | P0 | 人类基准对照组，固定时间间隔投入固定金额 |
| **MAS 小队** | P0 | 多 Agent 协作系统（PA + 技术分析员），核心验证对象 |
| **LLM 单兵** | P1 | 纯 LLM 决策对照组（无 MAS 架构） |
| **自定义 MAS** | P2 | 可配置策略参数的 MAS 小队 |

**验收标准**:
- [ ] 定义统一的 `Contestant` 接口
- [ ] 参赛者可以独立初始化、接收行情、返回交易决策
- [ ] 参赛者之间互不干扰，公平竞技
